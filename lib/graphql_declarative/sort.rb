...
module GraphqlDeclarative
  ...
  # them apart if `id` is not part of the ORDER BY, so page 2 either repeats or
  # skips the tied rows. The tiebreaker goes in the *same* direction as the sort
  # column, because Cursor.seek compares both halves in that direction.
  #
  ...
  class Sort
    ...
    # Always the last ordering term, and the second half of every cursor.
    TIEBREAKER = :id
    ...
    #   sort declared" and orders by :id, which SPEC.md §6.7 makes the default.
    ...
    def self.apply(scope, allowed:, field: nil, direction: :asc)
      ...
      field = resolve_field(model, allowed, field)
    ...
    #   needs it to build cursors, and it may differ from `field` (nil => :id).
    def self.column_for(scope, allowed:, field: nil)
      resolve_field(model_for(scope), allowed, field)
    end
    ...
    # raise: an unsortable field must not silently fall back to :id, or a client
    # gets a page ordered differently from the one its cursor was issued under.
    def self.resolve_field(model, allowed, field)
      return TIEBREAKER if field.nil? || field.to_s.empty?

      requested = field.to_s
      # :id is implicitly sortable — it is the tiebreaker on every other sort.
